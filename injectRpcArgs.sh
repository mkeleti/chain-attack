#!/bin/bash
docker exec --privileged RPC-Node geth attach /root/.ethereum/geth.ipc --exec "loadScript('/root/preload.js')"
run_attack()
{
  echo "stop miner"
  docker exec --privileged $1 geth attach /root/.ethereum/geth.ipc --exec "miner.stop()"
  sleep 1
  docker exec --privileged $1 killall -HUP geth
  docker exec --privileged $1 geth removedb
  case $1 in 
        Miner-Node)
  ...
  echo "import"
  docker exec --privileged $1 geth attach /root/.ethereum/geth.ipc --exec "admin.importChain('/root/backups/4')"
